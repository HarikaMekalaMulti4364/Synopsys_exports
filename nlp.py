...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for i in range(max_examples):
        for result in all_results:
            unique_id_to_result[result[i].unique_id] = result
...
#     for i in range(max_examples):
    ...
    # # print("\n unoque id to index", unique_id_to_result)
    # for key in unique_id_to_result.keys():
    #     print(key)
    # exit()
    ...
    # results = load_from_tensors(start_logits, end_logits, eval_features)
    # write_predictions(eval_examples, eval_features, results,
                    #   20, 30, True, out_file,2)
    # return results
...
def nlp(results_out, dataset_file, max_examples=None):
    ...
    res_fin = []
    i=0
    for results, pred in results_out.items():
        print("\n nlp len", len(pred[0]))
        # print("\n pred", pred[0])
        res_fin.append(pred[0])
        # res_fin = res_fin[1:]
        # results = pred
        # results_list.append(pred[0][])
        print("\n (pred[0][i+1] ", len(res_fin))
        # print("\n nlp len", len(results_list))
        details = results_out[results]
        eval_examples = details[2]
        max_inputs = details[1]
        eval_features = details[3]
    write_predictions(eval_examples, eval_features, res_fin,
                    20, 30, True, prediction_file, max_inputs)
    
    max_examples = 2
    # prediction_file = prediction_file["output_file"]
